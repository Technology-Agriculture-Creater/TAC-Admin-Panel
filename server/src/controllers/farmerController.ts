...
export const sendRegisterOtp = async (req: Request, res: Response) => {
  try {
    ...
    // Check if farmer already registered (and is verified)
    const existing = await Farmer.findOne({ mobileNumber }).exec();
    if (existing) {
      return res.status(400).json({
        success: false,
        message: 'Mobile number already verified/registered',
      });
    }
    ...
    const smsResponse = await axios({
      ...
      params: {
        route: 'q',
        message: `Hi Farmer 👋 Your TAC verification code is ${otp}. It expires in 5 mins.`,
        numbers: mobileNumber,
      },
    });

    if (smsResponse.data?.return !== true) {
      console.error('❌ Fast2SMS response error:', smsResponse.data);
      return res.status(502).json({
        success: false,
    ...
    console.error('❌ Error in sendRegisterOtp:', error?.response?.data || error);
    return res.status(500).json({
      success: false,
...
export const registerWithOtp = async (
  ...
  try {
    ...
    console.log("⬅️ registerWithOtp body:", req.body);
    ...
    // ---------------- DUPLICATE CHECK ----------------
    const existed = await Farmer.findOne({ mobileNumber }).lean();
    if (existed) {
      return res.status(400).json({
        success: false,
    ...
    if (!record || record.type !== "otp") {
      return res.status(400).json({
        success: false,
        message: "No OTP sent or invalid attempt.",
      });
    }
    ...
    delete registerOtpStore[mobileNumber]; // OTP verified

    // ---------------- USER ID GENERATION ----------------
    let userId = providedUserId;

    if (!userId) {
      const last = await Farmer.findOne({}, { userId: 1 }).sort({ createdAt: -1 }).lean();
      ...
      userId = `TAC${number.toString().padStart(5, "0")}`;
    }
    ...
    // Avoid Mongoose cast errors
    if (typeof userId !== "string") {
      return res.status(400).json({
        success: false,
        message: "Invalid userId generated.",
      });
    }
  ...
  try {
    ...
    const result = todayData.map((t) => {
      ...
      return {
        ...
        maxPrice: Math.round(t.maxPrice),
        priceChange: Math.round(priceChange),
        priceChangePercent: +(yesterdayAvg ? (priceChange / yesterdayAvg) * 100 : 0).toFixed(2),
        trades: +t.trades.toFixed(2),
        image: t.image || null,
        dateUsed: dateToUse,
      };
...
export const getTopGainers = async (req: Request, res: Response) => {
  try {
    ...
    // Compute gainers
    let gainers = await Promise.all(
      todayData.map(async (t) => {
        ...
        // 🔥 Fetch full crop document to get _id + image
        const cropDoc = await CropModel.findOne({
          name: t._id.name,
          'variants.name': Array.isArray(t._id.variant) ? t._id.variant[0] : t._id.variant,
          'location.city': t._id.city,
        }).lean();

        return {
          _id: cropDoc?._id, // ✅ Added
          image: cropDoc?.variants?.[0]?.image || null, // ✅ Added
    ...
    gainers = gainers.filter((c) => c && c.change > 0).slice(0, limit);
  ...
  try {
        ...
        },
        {
          $group: {
            _id: {
              ...
              variant: '$variants.name',
              city: '$location.city',
              docId: '$_id', // ⭐ add document ID
              image: '$variants.image', // ⭐ add variant image
            },
            avgPrice: { $avg: { $arrayElemAt: ['$variants.price', 0] } },
          },
        },
    ...
    let losers = todayData
      .map((t) => {
        ...
        return {
          _id: t._id.docId, // ⭐ return document id
          image: Array.isArray(t._id.image) ? t._id.image[0] : t._id.image, // ⭐ return image
          cropName: t._id.name,
          variantName: Array.isArray(t._id.variant) ? t._id.variant[0] : t._id.variant,
          location: t._id.city,
          avgPrice: Math.round(t.avgPrice),
...
// GET /api/commodities/arrivals
export const getArrivalsList = async (req: Request, res: Response) => {
  try {
    const city = (req.query.city as string)?.trim();
